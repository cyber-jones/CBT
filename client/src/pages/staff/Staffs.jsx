  ...
  return (
    <div className="p-6 font-sans h-full bg-green-100">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => navigate(cbt_url.staffRegistration)} className="btn btn-success">
          + Register Staff
        </button>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-scroll h-11/12">
        <table className="w-full text-left text-[10px] md:text-sm">
          ...
          <tbody>
            {staffs.map((staff) => (
              <tr onClick={() => navigate(cbt_url.staff+"/"+staff.id)} key={staff.id} className="border-b hover:cursor-pointer hover:bg-gray-50">
                <td className="p-3 font-medium text-gray-800">{staff.firstName}</td>
                <td className="p-3 text-gray-800">{staff.lastName}</td>
                <td className="p-3 text-gray-800 hidden lg:block">{staff.middletName}</td>
                <td className="p-3 text-gray-800">{staff.email}</td>
              </tr>
            ))}
          </tbody>
        </table>
